...
- `run_concurrently` 会把标记为 `is_concurrency_safe` 的工具批次执行。
...
demo 为了清楚起见采按顺序 dispatch。真实的 runtime 会把安全调用批次化，并按需加载庞大的工具 schema。
...
**粒度：**假设 agent 要读 PDF、Word 和试算表，该给它一个工具，还是三个？

功能相同、接受的输入格式也相同，就适合合并。一个带有类型参数的 `read_document`，会比三个外观相近的读取工具更容易选择。
...
Claude Code 和 Cursor 各站这张表的一端。Claude Code 不建索引，它一步一步搜：先 glob、再 grep、然后读档，模型在每次调用之间把查询收窄。
书里描述 Cursor 反过来，花成本建索引，好让一句白话查询也能找到没有指名任何识别字的代码。
...
| **编辑器指令** | 一套小型指令语言，vim 那种。 | 很精简。但又多一套语法可以写错。 |
...
**提早启动与失败范围：**一个调用不必等整批都写完。只要自己的参数解析完，它就可以先跑。
...
- **共享一个常驻 session：**书里把共享一个终端机当成默认，`cd`、export 出去的变量、启用中的虚拟环境全都留得住。
  要并行做事时，另外开几个隔离的 shell 就好。模型少打很多重复的配置指令，harness 则多了一份 session 状态要追踪和重置。
...
**对 cache 友善的加载：**加载进来的 schema 放在 context 的哪个位置，决定了它要花多少成本。
...
| --- | --- | --- | --- |
| **优点** | 每个工具各自带验证、权限、安全并行和延迟探索。 | 单一 `bash` 工具小得多，也没有目录要维护。 | 每个 agent 有自己的工具集，每次调用都走同一条可审计的 pipeline。 |
| **限制** | 每个工具都得背一份契约。 | 验证和权限做不到 per-tool。gate 看到的只有一条指令字符串。 | 再简单的工具也得声明 output 契约。 |
| **设计原因** | 添加一项能力，应该就只是注册一个工具，loop 维持不变。 | 假设每个行动都能写成一条 shell 指令，所以一个工具就够了。 | 同一个 scope 的可见性解析，同时喂给查表、dispatch 和呈现。 |
| **做法：tool definition** | schema、handler 与判定式。 | 一份 `bash` schema，只有一个指令字段，别的名称一律报错。 | schema、类型化的 output 契约、执行本体，加上纯函数的呈现器。 |
| **做法：dispatch** | 依名称查表，含别名。工具池依权限筛选，并合并 MCP 工具。 | 没有 registry，每次调用都是一条 shell 指令。 | 先做 scope 感知的查表，再走五阶段的守卫 pipeline。 |
| **做法：parallel calls** | 安全调用批次执行，不安全的单独执行。安全标记默认关闭。 | 没有。文字模式每次响应只允许一个 action。 | 每次调用都先分类，不确定就 fail closed 当 exclusive。 |
...
- **不安全的并行：**两个写入可能损毁同一个文件。默认采按顺序执行，除非确知某工具是安全的。
...
- **input 被偷偷改掉：**harness 在送进 handler 的路上做了标准化，或多塞了一个参数。调用失败了，模型却查不出原因。输入不合法就带着理由拒绝。
...
- [ai-agent-book](https://github.com/bojieli/ai-agent-book)：`book/chapter4.md`、`book/chapter5.md`（《深入理解 AI Agent》，李博杰；以中文原版为准）：
  五类工具的分法、粒度、description 的写法、参数保真、感知工具的接口规则、主动探索、对 cache 友善的加载、
  流式式提早启动与只中止依赖项、常驻 shell 默认、搜索与编辑的比较，以及检查用参数。
