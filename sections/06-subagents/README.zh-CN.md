...
这样做能让旁支调查不进到 parent 的 context。parent 不需要看到 child 读过的每个文件和每段命令输出，通常只需要最后的结论。

没有 subagent，所有调查过程都会堆在主 transcript 中。跑得越久，过程就越杂乱、成本越高，模型也更难跟上。
...
返回给 parent 的只有 child 最后输出的文字，child 的 transcript 不会一并带回。文件写入和 shell 副作用仍然会发生在工作目录里。
...
| **限制** | parent 不知道 child 是怎么得出答案的，摘要太简略就得再问一次。 | 一个工具能解决的事，这里有六种后端加一套续跑管理。 |
...
| **做法：result return** | child 最后一条消息的文字返回给 parent，transcript 丢弃。 | 最后一则 assistant 消息，外加可选的输出，会照 schema 检查。 |
| **做法：resume** | 多数 agent 可以续跑，parent 再发一条消息就好。 | durable 的 child 会把后续消息排进队列，重启后也能从 log 重新载回。 |
...
- **失控递归：**child 生 child 可能无上限地成长。从 child registry 省略 `Agent` tool，或强制设一个深度上限。
...
- [`test.py`](src/test.py)：检查全新 context、继承的权限，以及递归防护。
