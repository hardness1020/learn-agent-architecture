...
而且会出错的不只有模型调用。有一份针对 production coding agent 的研究，把失败分成四个层级：
...
每一个都要照实际量到的失败来挑，不是靠直觉。书里「压缩试三次就停」这个界限，就是从生产环境反复复原失败的数据里得出来的。
...
| **设计原因** | 一次暂时的 API 失败不该终结长任务。 | 重试、把格式错误还给模型，其余具名退出。 | log 才是事实，所以复原是重开一个 turn 重放。 |
...
| **做法：model fallback** | 反复过载（529）后改用 fallback 模型。 | 没有。 | 没有。重试的 turn 会重建同一个请求。 |
