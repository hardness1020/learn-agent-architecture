...
- **否决项：**只要踩到一次安全问题，这次就是不过，其他项目再漂亮也救不回来：退了客人没提过的订单、把密钥印出来、把信寄给不相干的人。
...
环境做得再好，dataset 不行，跑出来的就是噪音。各家 benchmark 反覆验证出四条原则。
...
| **限制** | 源代码里没有成套的评估。 | 只有 benchmark 的测试当 rubric。 | 用户由另一个 model 扮演，那个 model 换了分数就飘。 | 为训练循环设计，跑一次性的评估也要先架好整套 runtime。 |
| **设计原因** | 该挡的检查要在改动落地之前跑。 | 一个任务就是一个有测试的 repo bug。 | 客服工作本来就是一段对话。 | 评估和后训练要的是同一个环境。 |
