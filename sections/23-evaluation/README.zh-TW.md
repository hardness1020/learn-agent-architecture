...
> 測試環境撐得起多少，pass rate 就只值多少。

第 20 章記錄 production 裡發生過什麼，但光有紀錄，看不出來做得好不好。本章回答的是另一個問題：這次改動有沒有讓 agent 變好。

只有一次 model call 的時候，評估很單純：丟一個 prompt 進去，把回答跟標準答案比一比，再算答對幾題。

換成 agent，這套做法每一項都會壞掉。它會跑好幾輪對話，會反過來問使用者題目裡沒寫的資訊，也會呼叫 tool 去改存起來的資料。同樣的結果可以有好幾種走法，而且同一份 build 跑同一題，兩次的結果也可能不一樣。

所以要替 agent 打分，光有一串 prompt 不夠，得先搭一個測試環境：可以 reset 的 state、一個代打的使用者、負責推進對話的 protocol，還有一份拿執行完的狀態來打分的 rubric。

這些不做，分數照樣跑得出來，只是不代表什麼。題目可能早就流進訓練資料裡；新版本高出的 3 個百分點可能只是抽樣的波動；也可能分數很漂亮，結果它退的是客人根本沒提過的那筆訂單。
...
第 20 章的評估介面是 `(input, grade)`：輸入一個字串、取得一個字串，再算出 pass rate。本章保留同一個入口，只是入口後面接上一整套評估環境。
...
1. **照專家的標準寫：** 寫進去的是這個領域的專家真正會檢查的東西，不是文字通不通順。
...
評判者本身就是 model，model 有的毛病它都有：回答越長越容易拿高分，先讀到的那一份也容易被它挑中。
...
- **把 calibration 當成準確率（Calibration read as accuracy）：** calibrate 得很好的 gate，在單獨一題上照樣可能判錯。
  在一批樣本裡每題都選對路的 gate，calibration 也可能很差。這兩個指標誰都取代不了誰。
