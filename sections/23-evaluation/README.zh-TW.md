...
| **限制** | 原始碼裡沒有成套的評估。 | 只有 benchmark 的測試當 rubric。 | 使用者由另一個 model 扮演，那個 model 換了分數就飄。 | 為訓練迴圈設計，跑一次性的評估也要先架好整套 runtime。 |
| **設計原因** | 該擋的檢查要在改動落地之前跑。 | 一個任務就是一個有測試的 repo bug。 | 客服工作本來就是一段對話。 | 評估和後訓練要的是同一個環境。 |
