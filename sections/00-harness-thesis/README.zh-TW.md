...
- 第 8 章與第 9 章加上 context 管理與 memory。
...
接著逐層檢查：這項能力應該由模型還是 harness 負責？如果模型自己就會判斷，這一層就是多餘的，只是在浪費 token。
...
- **把模型的策略當成 harness 的設定：**什麼時候停止蒐集資訊，是模型學來的。prompt 規則只能推它一點。這一層要不要留，量過再決定。
