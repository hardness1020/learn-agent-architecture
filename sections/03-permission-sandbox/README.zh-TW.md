...
原因很簡單：模型會讀到別人寫的文字。網頁、issue 留言或 repo 裡的檔案，都可能暗藏操控 agent 的指令。
...
**沙箱擋掉哪些東西：**gate 也會判斷錯。沙箱的作用，就是讓判斷錯的那次 `allow` 不要付出太大代價。三個限制就擋掉大部分風險。

- **Egress：**網路預設擋掉，放行的流量走一個握有 host 允許清單的 proxy。
  三項能力裡，這一項最容易切掉。agent 照樣讀程式碼、照樣寫檔案，只是哪裡都送不出去。
- **Mount：**原始碼用唯讀掛載。憑證檔案一個都不要掛。只給一個可寫的工作目錄，其他都不給。
  agent 打不開的檔案，就外洩不出去。
- **Quota：**CPU、記憶體、磁碟、wall clock 時間都設上限。踩到上限的時候，回一個錯誤當 tool result，
  不要悄悄把 process 殺掉。模型讀得到 timeout，就知道換一條短一點的指令。悄悄殺掉，它什麼都讀不到。
...
| --- | --- | --- | --- |
| **優點** | mode、有序規則與沙箱化提供精確的控制。 | 幾分鐘就能稽核完。拒絕會回饋給模型。 | 拒絕只會收緊，沙箱判定 fail closed。 |
...
| **做法：rule persistence** | 規則依優先序合併，可存到 session 或 settings。 | config 裡的 regex，符合就跳過確認。 | 旋鈕變動是 log 事件，重放折疊出政策。 |
...
- **被核准的呼叫照樣外洩：**每一次呼叫單獨看都過得了 gate，整個 session 合起來卻還是讀到機密又把它送出去。
  網路預設就擋掉，第三種能力根本沒得用。
- **通過驗證但具破壞性：**砍掉重建也能通過結果檢查，因為終態是對的。要檢查的是動作，不是只有終態。
