...
> loop 会反覆调用模型、执行工具，再把结果送回去，直到这一轮完成。

一般的模型调用就是一问一答：送出 messages，收到一次响应，流程便结束。
...
从发问到收到完整答案的整段流程，就是**内层 loop**，也就是一个用户轮次（turn）。它会调用模型、检查 `stop_reason`、视需要执行工具并加入结果，直到模型给出最终答案。

接着你在同一个窗口再问「那明天呢？」，这就是新的一轮。
负责把多个轮次串成完整对话的，则是**外层 loop**。每一轮都会加到同一份 `messages[]`，所以当你接着问「那明天呢？」，模型仍然知道前面谈的是台北天气。
