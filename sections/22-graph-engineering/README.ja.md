...
エッジとは、書き下せる述語のことです。ですが、そう書けない分岐もあります。このコマンドは破壊的か。このチケットは課金の話か。
分岐が harness の担当だとは分かっているのに、コードで表せません。そこでまたモデルに投げ返すことになり、ルーティングに 1 turn 分のコストがかかります。

その中間に、3 つ目の種類のエッジがあります。狭い問いを 1 つ尋ね、数値を 1 つ受け取り、その数値でコードが分岐します。
...
これでルーティングがモデル側に戻るわけではありません。`route` はただのコードで、分岐を選ぶのは変わらずコードです。
この呼び出しで買えるのは数値 1 つ、コードだけでは計算できなかった数値です。本節冒頭の 2 点目はそのまま成り立ちます。ここはその下にある裏付けです。

- **しきい値は 1 つではなく 2 つ。** しきい値が 1 つだと、どの入力にも判定を強います。答えがいちばん当てにならない入力にもです。
  しきい値が 2 つあれば真ん中に帯が残ります。その帯こそ、harness が手を止めて人に尋ねるところです。
- **この層は狭めるだけ。** グラフが既に描いた分岐に点を付けるだけです。分岐を新しく与えることはありません。
  key がない、timeout した、答えの形が壊れている。どれも数値が返らないので `ask` にルーティングされ、検査が壊れても harness の既定の動きに落ちます。
- **confidence は締める方向にしか効かない。** 平らな答えは判定を `ask` の側へ動かします。`allow` の側へ動かすことはありません。

これを毎ステップ動かせるほど安くしているのが decision model です。TypeSafe の Jev は、state 1 つと型付きの問いの集合を受け取り、
問いごとの確率と、その答えがどれだけ尖っていたかを返します。テキストも、chain of thought も、パースするものもありません。
1 回のリクエストに入れた問いはすべて 1 パスで答えが出るので、5 つ尋ねても 1 つ尋ねるのとだいたい同じコストです。
エッジが成り立つのはそのおかげです。この呼び出しの値段は、turn ではなくコードの側に付いています。

ただし限界が 3 つ付いてきます。3 つとも提供元自身が挙げているものです。型付きの答えは形が整っているだけで、正しいこととは別です。
較正は答えの集団についての性質であって、目の前の 1 つの答えについての性質ではありません。
そして state は敵意ある入力ではなくデータとして読まれるので、答えを誘導するために書かれた文章が答えを動かせます。

ですからこれは置き換えではなく、段を重ねる話です。名前を付けられる分岐にはコードの規則、名前を付けられない長い裾には型付きの呼び出し、
その間の帯には人。セクション 3 の権限レイヤが規則の段です。ここで述べているのは、そのひとつ下の段です。
...
- 型付きのエッジの良し悪しは、2 つのしきい値で決まります。それは誰かが選んだ定数です。セクション 23 は、ラベル付きのログに照らしてそれを採点します。
...
- **確率を証明として読む。** 型付きの答えはいつも形が整っているので、間違っているときでも決着が付いたように見えます。
  緩和策: 判断を保留する帯を残し、この層には分岐を削らせるだけにして、分岐を与えさせないようにします。
- **agent 自身が書ける証拠。** 型付きのエッジに渡す state にはモデル自身の出力が混ざるので、実行が自分の gate を自分の言い分で通り抜けてしまいます。
  緩和策: その state は harness が持つフィールドだけで組み立て、この層は security の境界ではなく router として扱います。
- **しきい値を一度きりで決める。** あるモデルのバージョンとあるトラフィックの構成に合わせた定数 2 つを、その両方が変わっていく間そのまま放置します。
  緩和策: 判断を実行には使わずログに残し、そのログから合わせ直し、問いの文面としきい値をまとめてバージョン管理します (セクション 23)。
...
- [`decide.py`](src/decide.py): `route` (2 つのしきい値と、その間で判断を保留する帯)、`decision_edge`、オフライン実行用の記録済みの asker、そして標準ライブラリの http を使う live 版。
- [`test.py`](src/test.py): 一本道の順序と状態のマージ、コードだけのルーティング、予算で止まる循環、agent ノードの訪問ごとに新しい `messages[]` になること、
  3 通りに分かれる帯、危険とラベル付けされたものに allow を出さないこと、答えがないときは `ask` に落ちること、判定の単調性、そして中断の予算のオフライン検査。
- [`demo.py`](src/demo.py): ルーティングされる実行を 1 回。コードノードが分類し、型付きのエッジが gate になり、agent ノードが答え、
  セクション 21 の checker が採点し、不合格の判定はフィードバックとともに循環して戻ります。
  `TYPESAFE_API_KEY` が設定されていなければ gate は記録済みの答えを読むので、demo に要る key は今までどおり Anthropic のものだけです。
...
- [TypeSafe Jev docs](https://docs.typesafe.ai/api.md): リクエストの契約 (state 1 つと、型付きの問いのマップ) と、
  答えの形 (選択肢ごとの確率と confidence)。
  敵対的な state と context rot については [Limitations](https://docs.typesafe.ai/model-jaggedness/jev-1.13)、
  適用範囲の線引きについては [System One concepts](https://docs.typesafe.ai/concepts/how-to-build-with-system-one)。
  agent のためではなくソフトウェアのためのモデルであり、自分の次の行動を自分で選ぶことはありません。
  製品ページにあるレイテンシと価格の主張は提供元自身による実測で、第三者の再現はないので、このセクションは数字ではなく契約と限界を引いています。
