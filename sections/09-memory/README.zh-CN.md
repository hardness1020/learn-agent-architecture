...
memory 就是文件存储区加索引，再加上按需 recall。
...
- 查询字词以 `OR` 相连，任何一个字都能命中；`ORDER BY rank`（bm25）把最佳结果排在最前面。
  这就是带模糊排序的关键字 recall，跟 Hermes `session_search` 的做法一样。
...
`Store` 是 loop 用来操作 memory 的 handle，现在会在执行结束时同时写入两个存储区：
...
| **限制** | 每次 recall 都多一次模型调用。consolidation 需要另外一套管控。 | 关键字 recall 比较不精准。新写入要等下一个 session 才会进 prompt。 |
| **设计原因** | 什么都存，回想就会杂乱，所以 selector 每次只注入少数几个记忆。 | extraction 可能漏掉事实，所以把原始历史留成第二个存储区，随时搜得到。 |
| **做法：store** | 带 frontmatter 的 Markdown 文件。MEMORY.md 是索引，不是记忆内文。 | 两个 markdown 文件（观察记录和用户档案），加一份 SQLite session log。 |
| **做法：recall** | 模型读索引，最多选出 5 个记忆。内文注入时附上新鲜度注记。 | session 开始时把快照冻结进 prompt，过往 session 用关键字搜。 |
| **做法：extraction** | 分叉出的 agent 在执行结束时写入新的记忆文件。 | memory tool 在 session 中途把条目写进磁盘。写入可以先暂存等待批准。 |
...
- **过时记忆被当成事实：**带上写入时间或新鲜度的元数据。
- **存储区变杂乱：**合并重复项与相互矛盾的项目。
- **存储可推导的事实：**不要存储 grep、git 或源代码文件能回答得更好的事实。
- **Extraction 漏掉细节：**压缩可能在 extraction 之前就把细节磨掉了。在接近执行结束时抽取，并把重要事实留在文件里。
...
- [ai-agent-book](https://github.com/bojieli/ai-agent-book)：`book/chapter3.md`，以中文原著为准。文件系统即知识的范式。
